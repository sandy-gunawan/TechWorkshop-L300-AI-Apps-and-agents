...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async def run_customer_loyalty_task(customer_id):
        ...
        with tracer.start_as_current_span("Run Customer Loyalty Thread"):
            ...
            try:
                await websocket.send_text(fast_json_dumps({"type": "agent_status", "agent": "customer_loyalty", "status": "active"}))
            except Exception:
                pass
            ...
            try:
                await websocket.send_text(fast_json_dumps({"type": "agent_status", "agent": "customer_loyalty", "status": "done"}))
            except Exception:
                pass
    ...
    try:
        while True:
            ...
            try:
                ...
                # Signal handoff agent is active
                await websocket.send_text(fast_json_dumps({"type": "agent_status", "agent": "handoff_service", "status": "active"}))
                ...
                await websocket.send_text(fast_json_dumps({"type": "agent_status", "agent": "handoff_service", "status": "done"}))
                if not agent_name:
                    continue
            except Exception as e:
            ...
            try:
                ...
                # Signal selected agent is active
                await websocket.send_text(fast_json_dumps({"type": "agent_status", "agent": agent_name, "status": "active"}))
                ...
                # Execute agent
                await websocket.send_text(fast_json_dumps({"type": "agent_status", "agent": "mcp_tools", "status": "active"}))
                bot_reply = await execute_agent(
                    agent_name, agent_selected, agent_context,
                    project_client, tracer,
                )
                await websocket.send_text(fast_json_dumps({"type": "agent_status", "agent": "mcp_tools", "status": "done"}))
                await websocket.send_text(fast_json_dumps({"type": "agent_status", "agent": agent_name, "status": "done"}))
